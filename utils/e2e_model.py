...
class E2E_Model(nn.Module):
    ...
    def forward(
        ...
        # Convert channel outputs to NumPy for compatibility with Sionna
        y_np = to_numpy(y)
        h_np = to_numpy(h)

        # Receiver processing
        if self._training:
            losses = self._receiver([y_np, h_np, ebno_db, mcs_ue_mask, active_dmrs])
            return losses
        else:
            outputs = self._receiver([y_np, h_np, ebno_db, mcs_ue_mask, active_dmrs])

            if output_nrx_h_hat:
                b_hat, tb_crc_status, h_hat_refined, h_hat = outputs
                return (
                    (b, b_hat, tb_crc_status, h_np, h_hat_refined, h_hat)
                    if self._return_tb_status
                    else (b, b_hat, h_np, h_hat_refined, h_hat)
                )
            else:
                b_hat, tb_crc_status = outputs
                return (
                    (b, b_hat, tb_crc_status) if self._return_tb_status else (b, b_hat)
                )
