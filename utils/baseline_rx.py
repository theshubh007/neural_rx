...
def to_numpy(input_array):
    if isinstance(input_array, np.ndarray):
        ...
        raise TypeError(
            "Input type not supported. Please provide a NumPy array or PyTorch tensor."
        )
        ...
        **kwargs,
        ...
        if sys_parameters.system in ("baseline_lmmse_kbest", "baseline_perf_csi_kbest"):
            ...
            self._detector = KBestDetector(
                ...
                num_bits_per_symbol=sys_parameters.transmitters[
                    mcs_arr_eval_idx
...
class LowComplexityLMSEEstimator(nn.Module):
    def __init__(self, resource_grid, interpolator, sys_parameters, reduction=4):
        ...
        # Initialize LSChannelEstimator for pilot estimation
        self._ls_estimator = LSChannelEstimator(
            resource_grid, interpolator=interpolator
        )

        # static shapes
        self._num_pilots = self._sys_parameters.transmitters[
            0
        ]._resource_grid.num_pilot_symbols

    def forward(self, inputs):
        y, no = inputs
        y_eff = self._ls_estimator._removed_nulled_scs(y)
        y_eff_flat = torch.flatten(y_eff, start_dim=-2)
        y_pilots = torch.index_select(y_eff_flat, -1, self._ls_estimator._pilot_ind)
        h_hat, err_var = self._ls_estimator.estimate_at_pilot_locations(y_pilots, no)
        ...
        h_hat2 = h_hat.view(*h_hat.shape[:-1], 2, -1)
        h_hat3 = h_hat2.view(*h_hat2.shape[:-1], self._reduction, -1)
        h_hat4 = h_hat3.permute(6, 0, 1, 2, 3, 4, 5, 7)
        h_hat5 = torch.flatten(h_hat4, start_dim=-2)
        h_hat6 = torch.flatten(h_hat5, start_dim=2, end_dim=3)

        err_var2 = err_var.view(*err_var.shape[:-1], 2, -1)
        err_var3 = err_var2.view(*err_var2.shape[:-1], self._reduction, -1)
        err_var4 = err_var3.permute(6, 0, 1, 2, 3, 4, 5, 7)
        err_var5 = torch.flatten(err_var4, start_dim=-2)
        err_var6 = torch.flatten(err_var5, start_dim=2, end_dim=3)

        h_hat7, err_var7 = self._interpolator(h_hat6, err_var6)
        err_var7 = torch.clamp(err_var7, min=0)

        h_hat8 = h_hat7.view(self._reduction, -1, *h_hat7.shape[1:])
        h_hat9 = h_hat8.permute(1, 2, 3, 4, 5, 6, 0, 7)
        h_hat10 = torch.flatten(h_hat9, start_dim=-2)

        err_var8 = err_var7.view(self._reduction, -1, *err_var7.shape[1:])
        err_var9 = err_var8.permute(1, 2, 3, 4, 5, 6, 0, 7)
        err_var10 = torch.flatten(err_var9, start_dim=-2)
...
class LowComplexityPUSCHLMSEEstimator(PUSCHLSChannelEstimator):
    ...
    def forward(self, inputs):
        ...
        y_pilots = torch.gather(
            y_eff_flat,
            -1,
            self._pilot_ind.expand_as(y_eff_flat[..., : self._pilot_ind.size(-1)]),
        )
